...
def myPrint(*args):
    ...
    if args[0][0] == 'M':  
        global cache
        if  args[1] == 3: 
             cache.append(str(args[1]) + "\t" + str(args[3]) + "\t" + str(args[5]) + "\t" + str(args[9][0]) + "\t" + str(args[9][1]) + "\t" + str(args[9][2]) + "\t" + str(args[7]))
        elif testCounter >= num:
            cache[testCounter%num] = cache[testCounter%num] + "\t" + str(args[7])
...
def testMethod(fileName, method):
    ...
    for testCase in range(len(arrays)):
        ...
        if len(file) > 1:
            ...
            myPrint("Mthd", method, "found Case", testCase+1, "size=", len(array), "of 2th txt in", runtime, "as", result)
        global testCounter 
        testCounter+=1
...
for l in cache: 
    #output.write(l+"\n")
    l = l.split(sep='\t')
    ans.write(str(l[3]) + "\t" + str(l[4]) + "\t" + str(l[5]) + '\n')
array = [31, -41, 59, 26, -53, 58, -6, 97, -93, -23]   
half = (int)(len(array)/2)   
#array = [8, -3, 2, 1, -4, 10, -5]
#r = bonus(array)
#myPrint(r)