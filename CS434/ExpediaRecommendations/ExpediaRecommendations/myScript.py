...
def run_my_solution(TrainFile = "train.csv", testFile = "test.csv"):
    print('Preparing arrays...')
    f = open(TrainFile, "r")
    f.readline()
    best_hotels_od_ulc = defaultdict(lambda: defaultdict(int))
#
    best_hotels_ul_region_hotel_country = defaultdict(lambda: defaultdict(int))
    best_hotels_ul_region_hotel_dest = defaultdict(lambda: defaultdict(int))
#
    ...
    best_hotel_region = defaultdict(lambda: defaultdict(int))
    ...
    while 1:
        ...
        if s2 in best_hotels_search_dest:
            ...
            for i in range(len(topitems)):
                ...
                filled.append(topitems[i][0])
#
        s4 = (user_location_region, srch_destination_id)
        if s4 in best_hotels_ul_region_hotel_dest:
            d = best_hotels_ul_region_hotel_dest[s4]
            topitems = nlargest(5, d.items(), key=itemgetter(1))
            for i in range(len(topitems)):
                if topitems[i][0] in filled:
                    continue
        ...
        s3 = (user_location_region, hotel_country)
        if s3 in best_hotels_ul_region_hotel_country:
            d = best_hotels_ul_region_hotel_country[s3]
            topitems = nlargest(5, d.items(), key=itemgetter(1))
            for i in range(len(topitems)):
                if topitems[i][0] in filled:
                    continue
