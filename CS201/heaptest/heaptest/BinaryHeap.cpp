...
void BinaryHeap::percolateDown(int hole) {
	...
	int leftChild = 2 * hole, rightChild = 2 * hole + 1;
	if (rightChild <= size)
	{

		if (heap[hole] > heap[leftChild] && heap[hole] > heap[rightChild])
		{
		...
		else if (heap[hole] > heap[leftChild])
		{
			swap(hole, leftChild);
			percolateDown(leftChild);
		}
		else if (heap[hole] > heap[rightChild])
		{
			swap(hole, rightChild);
			percolateDown(rightChild);
		}
