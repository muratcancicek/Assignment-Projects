...
def evalJson(fileName, printing = True):
    ...
    data = eval(rawLines)
    if printing:
        print_(fileName + '.json has been read successfully by', nowStr())
    return data

def writeToJson(json, fileName,  printing = False, printText = False, decoding = sys.getfilesystemencoding(), sort = True, printLog = True): 
    f = open(fileName + '.json', 'wb')
    jsonString = jsonToString(json, printing, decoding, u'\n', sort = sort).encode(decoding)
    f.write(jsonString) 
    if printLog:
        print_(fileName + '.json has been written successfully by', nowStr())
    f.close() 

def writeToJsonRaw(data, fileName,  printing = False, printText = False, decoding = sys.getfilesystemencoding(), sort = True, printLog = True): 
