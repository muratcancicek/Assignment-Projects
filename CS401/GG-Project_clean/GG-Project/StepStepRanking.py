...
def findPairById(i1, i2, path, p = True):
    import Trainer
    pairs = Trainer.readLabeledPairs(path)
    pairs = pairs.filter(lambda p: (i1, i2) == p[0])
    import PythonVersionHandler
    if p: PythonVersionHandler.print_high_logging(pairs.count(), 'Instances have been found for this pair.')
    pairsP = pairs.filter(lambda p: 1 == p[1])
    if p: PythonVersionHandler.print_high_logging(pairsP.count(), 'Positive instances have been found for this pair.')
    pairsN = pairs.filter(lambda p: 0 == p[1])
    if p: PythonVersionHandler.print_high_logging(pairsN.count(), 'Negative instances have been found for this pair.')
...
def generatePairs(ids):
    ...
    for id1 in ids:
        for id2 in ids:
            if id1 != id2:
                pairs.append(id1, id2, findPairById(id1, id2, False))
    for p in pairs:
        pass