...
def generateTrainData(labeledPairs, products):  #TO DO
    ...
    features = labeledPairs.filter(lambda x: x[0][0] in products.keys() and x[0][1] in products.keys())\
        .map(lambda x: products[x[0][0]] - products[x[0][1]])
    PythonVersionHandler.print_high_logging(features.count())
